...
if ! [ -e /var/run/ansible-deploy.run ]; then
	...
	time ansible-playbook /var/tmp/nat-to-dns/deploy-stack.yml -e ec2_url=`head -1 /var/tmp/ec2.conf` -e s3_url=`head -1 /var/tmp/s3.conf` -e add_cron=false --skip-tags=packages
	pass=$((pass+1))
    done
